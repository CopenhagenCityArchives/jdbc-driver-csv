...
If True, unquoted line breaks are handled by reading several rows.

*tablesToAddPrimaryKey*

type: String

default: None

A comma-separated list of table names to which a column that enumerates the row numbers is prepended, which can be used as a primary key.
